...
int main(int argc, char **argv){
  ...
  for(unsigned p = 0; p < n_platforms; p++){
    cluGetPlatformInfo(platforms[p], CL_PLATFORM_NAME, CLU_DYNAMIC, &platform_name);
    ...
    for(unsigned d = 0; d < n_devices; d++){
      cluGetDeviceInfo(devices[d], CL_DEVICE_NAME, CLU_DYNAMIC, &device_name);
      report(INFO, "\tdevice %u: %s", d, device_name);
      cluGetDeviceInfo(devices[d], CL_DEVICE_BUILT_IN_KERNELS, CLU_DYNAMIC, &builtin_kernels);
